...
- `sphinx_examples_as_code_gallery_downloads`: opt-in takeover of
...
With `sphinx_examples_as_code_gallery_downloads = True`, this extension takes over the
downloads on every page generated by
[sphinx-gallery](https://sphinx-gallery.github.io): the "Go to the end to download the
full example code" note, the `.py`/`.ipynb`/`.zip` download footer, the "Total running
time" line, and the "Gallery generated by Sphinx-Gallery" credit line are all removed,
and replaced with download link(s) built by this extension instead — same conversion
rules as above, applied to the whole page rather than one Examples section. The timing
and credit lines are dropped rather than kept alongside the new link: a "Total running
time" printed next to a download button reads like it's timing *your* run, not the
build's; and a "Generated by Sphinx-Gallery" credit next to a file this extension
generated is simply wrong.

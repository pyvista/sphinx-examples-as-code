...
- `include_see_also`: whether "See Also" content is included in the generated file.
  `False` (default) excludes it, in every one of its forms. `True` includes it: a
  `.. seealso::` admonition, a bare `.. rubric:: See Also`, a hand-written `See Also`
  heading, or numpydoc's own "See Also" field (including when it's been reordered outside
  the Examples section itself, or the Examples section has been hoisted to a heading of its
  own — a setup some projects use to get "Examples" listed in the page's own navigation).

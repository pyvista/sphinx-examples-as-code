...
- `footer`: a string appended to the end of every generated file, enclosed in comments
  (a multi-line string becomes one comment line per line). Defaults to a "generated
  file" credit linking back to this project's issue tracker; set to `None` to omit it
  entirely. A blank line always separates it from the file's actual content. Any bare
  `http(s)://` URL in it is rendered as a real clickable link in `.ipynb` -- a `.py`
  comment can't be a hyperlink, so there it stays as plain URL text.

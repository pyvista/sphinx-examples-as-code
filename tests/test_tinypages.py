...
def test_default_footer_appears_in_py(tmp_path: Path):
    ...
    footer_idx = lines.index(_DEFAULT_FOOTER_PY)
    assert lines[footer_idx - 1] == ''  # blank line before the footer


def test_default_footer_appears_in_ipynb_with_clickable_link(tmp_path: Path):
    html_dir = tmp_path / 'html'
    doctree_dir = tmp_path / 'doctrees'
    returncode, out, err = _run_sphinx_build(_sphinx_build_cmd(SRCDIR, html_dir, doctree_dir))
    assert returncode == 0, f'sphinx build failed with stdout:\n{out}\nstderr:\n{err}\n'
    ...
    assert 'File generated by `sphinx-examples-as-code`' in all_source
    assert _DEFAULT_FOOTER_LINK in all_source
    # .py's plain URL (no markdown link syntax) should NOT appear in the notebook
    assert 'Report issues to https://github.com' not in all_source

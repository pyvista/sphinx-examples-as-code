...
def test_default_footer_appears_in_py(tmp_path: Path):
    ...
    footer_idx = lines.index(_DEFAULT_FOOTER_PY_LINES[0])
    assert lines[footer_idx - 1] == ''  # blank line before the footer
    assert lines[footer_idx : footer_idx + len(_DEFAULT_FOOTER_PY_LINES)] == list(
        _DEFAULT_FOOTER_PY_LINES
    )
...
def test_default_footer_appears_in_ipynb_with_clickable_links(tmp_path: Path):
    ...
    for link in _DEFAULT_FOOTER_IPYNB_LINKS:
        assert link in all_source

...
# to import sphinx_design or sphinx_gallery. Dropped entirely:
...
_SKIP_SUBTREE_CLASSES = ('sd-dropdown', 'sd-tab-set', 'sphx-glr-script-out')
...
_GALLERY_FURNITURE_CLASSES = (
    _GALLERY_NOTE_CLASS,
    _GALLERY_FOOTER_CLASS,
    _GALLERY_TIMING_CLASS,
    _GALLERY_SIGNATURE_CLASS,
)


def _strip_gallery_furniture(doctree: nodes.document) -> bool:
    """Remove sphinx-gallery's injected note/footer/timing/signature nodes, if present.

    The timing and signature lines are dropped rather than kept alongside
    our own download link: "Total running time" describes the *build's* run,
    not the reader's, and could easily be misread as such once the page also
    offers a download; the "Generated by Sphinx-Gallery" credit is
    misleading once this extension -- not sphinx-gallery -- is what produced
    the downloadable file being linked.
    ...
    """
    matches = list(
        doctree.findall(lambda n: any(_has_class(n, cls) for cls in _GALLERY_FURNITURE_CLASSES))
    )
